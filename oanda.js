...
function OandaClient (bearerToken, APIType) {
...
OandaClient.HOSTS = {
    ...
    sandbox: 'api-fxpractice.oanda.com'
};

OandaClient.prototype._post = function (endpoint, params, callback) {
    var options = {
        host: OandaClient.HOSTS[this.APIType],
        path: '/v1/' + endpoint,
        method: 'post',
        headers: {
            'User-Agent': 'Mozilla/4.0 (compatible; OANDA node.js client)',
 ...
 * lowerBound   => number
 */
OandaClient.prototype.open = function(accountId, instrument, units, side, type, optParameters, callback) {
    this._post('accounts/' + accountId + '/orders',
            _.extend({ instrument: instrument, units: units, side: side, type: type }, optParameters),
            callback);
};
